...
- Hardened against pathological input: cipher/extension/sig-alg/cookie lists are
  capped at 256 and header names at 100. Over-cap input is truncated to a
  deterministic fingerprint and logged at `warn`; real clients are unaffected and
  remain byte-identical to canonical JA4/JA4H.
...
- For live JA4 `compute()`: **OpenResty >= 1.29.2.1**, which bundles
  ...
  getters and are not supported. (`build()` on pre-parsed data works on any version.)
